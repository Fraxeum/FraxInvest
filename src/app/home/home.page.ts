...
import { ActionSheetController, AlertController, MenuController, IonFab, Platform } from '@ionic/angular';
...
export class HomePage {
  ...
  bannerImages: Array<BannerImage> = [
    { uri: 'assets/sections/sustainable_farming_banner.png', routerLink: '/investment-detail-two' },
    { uri: 'assets/sections/plastic_recycling_banner.png', routerLink: '/investment-detail-one' },
  ...
  toggleMenu() {
    ...
    this.menu.toggle("main").then(() => {
      console.log("opened");
    }, (err) => { console.log(err); });
  }

  ionViewWillEnter() {
    this.storage.getItem("session_token").then(
      async (token) => {
  ...
  async setDataStructures(): Promise<any> {
    return new Promise((resolve, reject) => {
      if (this.userdata) {
        this.sessionToken = this.userdata.token;
    ...
    const buttons = [
      ...
      },
      {
        ...
        handler: () => {
          ...
          this.launchExternalWebsite(url ? url : this.userService.websiteUrl);
        }
      },
      {
        text: 'Dismiss',
  ...
  async openModal() {
    ...
    this.helpStrings.getHtmlString(this.currentPage).then(async (text: string) => {
      ...
      helpScreen = await this.userService.getInstructionsPage(helpText);
