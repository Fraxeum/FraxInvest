...
export class MarketPage {
  ...
  /*
  getOfferStringSellOrders(index, side) {

    if (!this.allMySellOrdersList[index].offers || this.allMySellOrdersList[index].offers.length === 0) {
      return (side === "left") ? "-" : '';
    }
    ...
    if (this.allMySellOrdersList[index].offers.Counter.length === 1) {
      str = "R " + this.allMySellOrdersList[index].Counter[0].Price;
    }

    // last offer in array from buyer - return buyer last price
    if (!str && this.allMySellOrdersList[index].offers.Counter[this.allMySellOrdersList[index].Counter.length - 1].BS === 'B' && this.allMySellOrdersList[index].Counter.length > 0) {
      str = "R " + this.allMySellOrdersList[index].Counter[this.allMySellOrdersList[index].Counter.length - 1].Price;
    }
    ...
    if (!str && this.allMySellOrdersList[index].Counter[this.allMySellOrdersList[index].Counter.length - 1].BS === 'S' && this.allMySellOrdersList[index].Counter.length > 1) {
      str = "R " + this.allMySellOrdersList[index].Counter[this.allMySellOrdersList[index].Counter.length - 2].Price;
    }
    ...
    return side === "left" ? strArr[0] + "." : strArr[1];
  }
  */

  getCounterStringSellOrders(index, side) {
    /*
        let str = null;
        // tslint:disable-next-line:no-trailing-whitespace
        // last offer in array was counter offer by seller, return offer before this item 
        // each of buyer and seller alternate offers, one party cannot counter before the other has responded
        if (this.allMySellOrdersList[index].offers[].Counter[this.allMySellOrdersList[index].Counter.length - 1].BS === 'S') {
          str = "R " + this.allMySellOrdersList[index].Counter[this.allMySellOrdersList[index].Counter.length - 1].Price;
        }
  ...
  /* BACKUP IN CASE I BREAK SOMETHING ELSE 
  getBuyerOfferString(index, side) {allMyPurchaseOrderData

    let str = null;

    console.log("this.myOffersToPurchaseList["+index+"]");
    console.log(this.myOffersToPurchaseList);
    //console.log(this.myOffersToPurchaseList[index]);
    return;

    if (this.myOffersToPurchaseList[index].Counter.length === 1) {
      str = "R " + this.myOffersToPurchaseList[index].Counter[0].Price;
    }

    // last offer in array from buyer - return buyer last price
    if (!str && this.myOffersToPurchaseList[index].Counter[this.myOffersToPurchaseList[index].Counter.length - 1].BS === 'B' && this.myOffersToPurchaseList[index].Counter.length > 0) {
      str = "R " + this.myOffersToPurchaseList[index].Counter[this.myOffersToPurchaseList[index].Counter.length - 1].Price;
    }

    // last offer in array was counter offer by seller, return offer before this item 
    // each of buyer and seller alternate offers, one party cannot counter before the other has responded
    if (!str && this.myOffersToPurchaseList[index].Counter[this.myOffersToPurchaseList[index].Counter.length - 1].BS === 'S' && this.myOffersToPurchaseList[index].Counter.length > 1) {
      str = "R " + this.myOffersToPurchaseList[index].Counter[this.myOffersToPurchaseList[index].Counter.length - 2].Price;
    }
  ...
  getSellerCounterString(index, side) {

    let str = null;
    // last offer in array was counter offer by seller, return offer before this item 
    // each of buyer and seller alternate offers, one party cannot counter before the other has responded
    if (this.myOffersToPurchaseList[index].Counter[0].BS === 'S') {
      str = "R " + this.myOffersToPurchaseList[index].Counter[0].Price;
    }
  ...
  getSellerOfferOnSOString(index: number, side: string) {

    let str = null;
    // last offer in array was counter offer by seller, return offer before this item 
    // each of buyer and seller alternate offers, one party cannot counter before the other has responded
    if (this.offersOnSellorderList[index].Counter[0].BS === 'B') {
      str = "R " + this.offersOnSellorderList[index].Counter[0].Price;
    }
  ...
  async showCounterForm(uid: string, userid: string, index: number) {
    ...
    console.log("this.offersOnSellorderList[index]");
    console.log("This......");
    console.log(this.offersOnSellorderList);
    console.log("DONE");
  ...
  async showBuyOffersActionSheet(uid: string, userid: string, index: number) {
    /*
...
*/
  ...
  showActionSheet(uid: string, userid: string, index: number, asset?: string) {
    /*
        console.log("HOWZIT");
    ...
    const actionSheet = await this.actionSheetCtrl.create({
      ...
      buttons: [
        ...
        },
        {
          text: 'View company',
          handler: () => {
            const url = this.allSellOrderData[index] !== undefined ? this.allSellOrderData[index].CompanyWebsite : this.user.websiteUrl;
            this.launchExternalWebsite(url);
            console.log('view company clicked');
          }
        },
  ...
  async presentBuyerActionSheet(uid: string, index: number, asset: string) {
    /*
  ...
  async presentSellerActionSheet(uid: string, index: number) {
    /*
  ...
  async presentNewSellerActionSheet(uid, index) {
  ...
  initShowMyTrades(): boolean {
    const show = localStorage.getItem('showMyTrades');
  ...
  async getTradeOffers(index: number, userid: string, offerid: string, numResponses: number) {
    /*
        this.currentIndex = index;
        this.currentAsset = this.allSellOrderData[index].pair;
  ...
  //returns a list of offers received from various buyers on a specific trade
   async getMyOffersFromBuyers(index: number, asset: string) {
    this.currentAsset = asset;
    this.currentIndex = index;
  ...
  */
  ...
  /*returns a list of negotiation messages
   async getMyOffersToPurchase(offerId: string) {
  ...
  /* OLD REMOVE
    async loadData(asset: string, event: any) {
      await this.loadAllSellOrdersData(asset, "fwd", event);
      /*
  
      this.loadAllSellOrdersData(asset, "fwd", event).then(async (response) => {
        setTimeout(() => {
          console.log('Done');
  ...
  /*
    closeSlidingItems(item: string) {
      switch (item) {
        case 'a':
          if(this.lista.)
          this.lista.closeSlidingItems(); break;
        case 'b':
          this.listb.closeSlidingItems(); break;
        case 'c':
          this.listc.closeSlidingItems(); break;
      }
  ...
  */
  ...
  /*
    // opens a specific seller item for negotiation. buyOfferId is the buyer offer item id on a particular share sales offer 
    async openSellerOfferItem(index: number, sellOfferId: number) {
  ...
  */
  ...
  handleRowClick(uid: string, userid: string, index: number) {
    ...
    if (this.allSellOrderData[index] && this.allSellOrderData[index].ResponseCount && this.allSellOrderData[index].ResponseCount.open) {
      try {
        respCount = parseInt(this.allSellOrderData[index].ResponseCount.open, 10);
      } catch (err) { }
    }
    */
    ...
    if (userid === this.myMemberID) { // current user as buyer or seller
      // respCount > 0 ? this.getTradeOffers(index, userid, uid, respCount) :
      this.confirmCancelMySalesOrder(uid, index);
      return;
    }
